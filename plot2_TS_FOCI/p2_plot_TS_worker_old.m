function p2_plot_TS_worker(path,exp,region)

filelist= dir([path exp '/' '*_reg' num2str(region) '.nc']);
%year = 1980:2019;
...
% Colorbar
colormap(colours);
caxis([str2num(filelist(1).name(19:22))-.5 str2num(filelist(end).name(19:22))+.5]);
cb = colorbar;
set(cb,'fontsize',20);
...
pngname = (['plots/p2_' exp '_TS_coloured_time_REG_' num2str(region) '_' regional_settings.region_name '.png']);
